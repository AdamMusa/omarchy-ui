...
Item {
  ...
  function audioDevice(){var requested=String(renderer?renderer.prop("audio_device",""):"");var devices=mediaDevices.audioOutputs;for(var index=0;index<devices.length;index++)if(String(devices[index].id)===requested||devices[index].description===requested)return devices[index];return mediaDevices.defaultAudioOutput}
  ...
  AudioOutput { id:nativeAudioOutput;device:playerRoot.audioDevice();volume:Math.max(0,Math.min(1,Number(renderer?renderer.prop("volume",1):1)));muted:renderer&&renderer.prop("muted",false)===true }
  MediaPlayer {
    ...
    onErrorOccurred:function(error,message){playerRoot.send("error",{code:error,message:message})}
