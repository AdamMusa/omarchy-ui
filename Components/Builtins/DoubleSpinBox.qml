...
QQC.DoubleSpinBox {
  required property var renderer
      ...
      stepSize: Number(renderer.prop("step", 0.1))
      decimals: Number(renderer.prop("decimals", 2))
      ...
      textFromValue: function(value, locale) {
        return String(renderer.prop("prefix", "")) + Number(value).toLocaleString(locale, "f", decimals) + String(renderer.prop("suffix", ""))
      }
