...
static void drain_fd(mrb_state *mrb, int fd, mrb_value output, int *open_flag) {
  ...
  for (;;) {
    ...
    if (count > 0) mrb_str_cat(mrb, output, buffer, (mrb_int)count);
...
static mrb_value native_command(mrb_state *mrb, mrb_value self) {
  mrb_value command;
  mrb_float timeout = 0;
  mrb_get_args(mrb, "A|f", &command, &timeout);
  ...
  int out_open = 1, err_open = 1, status = 0, exited = 0, timed_out = 0;
  double started = monotonic_seconds();
  while (out_open || err_open || !exited) {
    struct pollfd fds[2] = {{out_pipe[0], POLLIN | POLLHUP, 0}, {err_pipe[0], POLLIN | POLLHUP, 0}};
    poll(fds, 2, 20);
    if (out_open && fds[0].revents) drain_fd(mrb, out_pipe[0], stdout_value, &out_open);
    if (err_open && fds[1].revents) drain_fd(mrb, err_pipe[0], stderr_value, &err_open);
...
void mrb_omarchy_ui_runtime_gem_init(mrb_state *mrb) {
  ...
  mrb_define_module_function(mrb, module, "native_command", native_command, MRB_ARGS_ARG(1, 1));
